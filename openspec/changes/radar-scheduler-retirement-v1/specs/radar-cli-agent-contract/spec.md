...
CLI SHALL 另外提供 `schedule show` 与 `schedule session-plan --runtime grok|claude|both`。`schedule install`（任意 backend）与 `market schedule install` MUST 以稳定错误码 `command_retired` 拒绝、零文件写入，错误信息指向客户侧接线文档。未知 backend/runtime MUST 以稳定错误码拒绝。`session-plan` MUST 走标准 envelope，且 MUST NOT 触发采集。
...
#### Scenario: 非法 backend
- **WHEN** 用户运行 `radar schedule show --backend cron`
- **THEN** CLI 以 `backend_invalid` 失败，不写任何文件

#### Scenario: install 退役
- **WHEN** 用户运行 `radar schedule install --backend cron`
- **THEN** CLI 以 `command_retired` 失败（不再解析 backend），不写任何单元文件，错误信息含接线文档路径
