...
- 增加 CLI/application service 市场能力，复用 Drizzle SQLite、输出 envelope、审计及运行证据。（原计划的可协商 MCP 市场面曾随软件门交付，已于 2026-09-15 随提交 5d8d78a 整体移除；外部消费统一走 CLI 合同 docs/interfaces/agent-cli-consumption.md。）
...
- `radar-market-agent-surface`：CLI 消费面、证据问答、任务恢复、观测与验证输出。
...
范围为 src/adapters、src/pipeline、src/db、src/app、CLI、调度及对应测试/文档，代码仍为 TypeScript/Bun＋Drizzle（src/mcp 已于 2026-09-15 随 5d8d78a 移除，不再属于实现面）。不新增服务端、商业数据采购、聊天投递、影片下载或生产能力。
