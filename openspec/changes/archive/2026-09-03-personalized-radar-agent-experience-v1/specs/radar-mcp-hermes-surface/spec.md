...
MCP SHALL 默认 reader，并按 reader < curator < operator 累积授权。`radar.search` 对所有 lane 可用；`feedback_add/opportunity_review` 至少需要 curator；`collect/score/cluster_build/edition_build/daily_run` 需要 operator。Profile create/set/activate MUST NOT 经任何 MCP lane 暴露。
