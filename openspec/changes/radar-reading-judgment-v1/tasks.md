...
- [ ] 1.2 owner=Radar；scope=可选 SDK consumer；depends=1.1,SDK 包；lane=integration；实现 off/shadow/assist 和注入 transport，默认 off；验收=off/replay 零调用且 unknown outcome 不自动重发；验证=bun test --timeout 30000；失败复验提交状态与授权。
