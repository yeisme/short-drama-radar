...
- 拥有：短剧候选发现、快照历史、跨平台标准化、钩子/题材/情绪标签、基础 market score/`confidence`、个人 Profile/revision、反馈 ledger、机会簇、personal fit、Morning Edition、卡片合同 payload、MCP 安全投影、运行/审计证据，以及调度建议计划、客户侧接线文档与跨进程运行锁（Radar 不生成/安装 OS 调度单元；`schedule install` 已退役）和 Claude/Grok session-plan 投影。不实现 Claude/Grok 调度器本身。
